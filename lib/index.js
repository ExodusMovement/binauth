...
const createBinauth = ({ serverPublicKey, serverPrivateKey }) => {
  ...
  const getChallenge = async ({ publicKey }) => {
    ...
    return challengeBuf.toString('base64')
  }

  const getToken = async ({ signedChallenge, publicKey }) => {
    signedChallenge = Buffer.from(signedChallenge, 'base64')
    publicKey = Buffer.from(publicKey, 'hex')
    ...
    return authToken.toString('base64')
  }
